...
COMMANDLINESET2;
	...
	function search_register_page() {
		...
		$command_line_html = '<a href="'.admin_url('admin.php?page=mediafromftp-settings#mediafromftp-settings-tabs-2').'">'.__('Command-line', 'media-from-ftp').'</a>';
		if ( !@set_time_limit($max_execution_time) ) {
			echo '<div class="error"><ul><li>'.sprintf(__('Execution time for this server is fixed at %1$s. Please note the "Number of files to be registered" or "Number of items per page" so as not to exceed this limit. Please use the %2$s if you do not want to be bound by this restriction.', 'media-from-ftp'), $limit_seconds_html, $command_line_html).'</li></ul></div>';
		}
